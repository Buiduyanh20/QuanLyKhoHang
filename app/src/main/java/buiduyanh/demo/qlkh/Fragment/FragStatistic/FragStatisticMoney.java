...
import buiduyanh.demo.qlkh.Adapter.StatisticAdapter;
import buiduyanh.demo.qlkh.Custom.NF;
...
public class FragStatisticMoney extends Fragment {
    FragStatisticsMoneyBinding binding;
    OrderDetailsDao dao;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        binding = FragStatisticsMoneyBinding.inflate(inflater, container, false);
    ...
    public void show() {
        int s = 0;
        for (Product prod : dao.getStatistic(0, time, 1)) {
            s += prod.getPrice();
        }
        binding.tvIncome.setText(NF.format(s));
        s = 0;
        for (Product prod : dao.getStatistic(1, time, 1)) {
            s += prod.getPrice();
        }
        binding.tvExpense.setText(NF.format(s));
        //Xử lý list view
        list = dao.getStatistic(type, time, 1);
        adapter = new StatisticAdapter(list, 1);
        binding.rcv.setAdapter(adapter);
    }

    public void showDiff() {
        int s = 0;
        for (Product prod : dao.getStatisticDiff(0, selection, 1)) {
            s += prod.getPrice();
        }
        binding.tvIncome.setText(NF.format(s));
        s = 0;
        for (Product prod : dao.getStatisticDiff(1, selection, 1)) {
            s += prod.getPrice();
        }
        binding.tvExpense.setText(NF.format(s));
        //Xử lý list view
        list = dao.getStatisticDiff(type, selection, 1);
        adapter = new StatisticAdapter( list, 1);
        binding.rcv.setAdapter(adapter);
    }
